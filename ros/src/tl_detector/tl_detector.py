...
#        self.waypoints = waypoints
    ...
    def process_traffic_lights(self):
        ...
        # TODO find the  visible TL closest to stop_line_index by looping through
        # all lights
        for q in range(TL_array):
            #rospy.logwarn('idx is %s total pts %s   array %s' , q, len(self.base_waypoints),TL_array)
            dist = dl(self.base_waypoints[stop_line_idx].pose.pose.position, self.lights[
        ...
        if ((self.count % 5) == 0):
            ...
            rospy.logwarn('Light # %s coming in %f m \n\n\n', closest_light_index, dl(
                self.lights[closest_light_index].pose.pose.position, self.pose.position))
